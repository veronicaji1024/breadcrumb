...
class NotchViewModel: ObservableObject {
    ...
    /// Whether we're in a sticky content mode (doesn't close on notch click)
    private var isInChatMode: Bool {
        if case .chat = contentType { return true }
        if case .historySearch = contentType { return true }
        return false
    }
    ...
    /// Show history search panel
    func showHistorySearch() {
        contentType = .historySearch
    }

    /// Boot animation disabled — stay closed until user explicitly opens
    func performBootAnimation() {
        // No-op: don't auto-expand on launch
    }
}
