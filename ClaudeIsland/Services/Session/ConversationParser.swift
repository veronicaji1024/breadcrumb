...
actor ConversationParser {
    ...
    /// Falls back to scanning project directories if exact match fails (handles non-ASCII path encoding)
    private static func sessionFilePath(sessionId: String, cwd: String) -> String {
        ...
        let directPath = projectsBase + projectDir + "/" + sessionId + ".jsonl"
        ...
        // Fallback: search all project dirs for this session file
        // This handles cases where Claude Code encodes non-ASCII chars differently
        if let dirs = try? FileManager.default.contentsOfDirectory(atPath: projectsBase) {
            for dir in dirs {
                let candidate = projectsBase + dir + "/" + sessionId + ".jsonl"
        ...
        // Return direct path even if not found (caller handles missing file)
        return directPath
    }
